define([], function () {
	...
	var factory = function (NA, $) {
		...
		NA.getWordList = function () {
			var wordList = $("#word-list").val().split("\n");
		...
		NA.wordListValid = function () {
			var wordList = NA.getWordList().sort();
					...
					$.get("../browse", {url:"http://dictionary.cambridge.org/search/english/direct/?source=gadgets&q=" + v},
						(function(i, v, output) {
							return function(data) {
								...
								var entry = out_eg.parents(".entry-body__el");
								var out = [];
	...
	requirejs(["jquery"], (function (NA, factory) {
